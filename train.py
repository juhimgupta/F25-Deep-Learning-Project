    ...
    def __getitem__(self, index):
        ...
        for _ in range(max_attempts):
            try:
                ...
                logger.warning(f"Skipping corrupted image at index {index}: {e}")
                # Try next index
...
def main():
    ...
    """
    #uniform scale transforms for 128x128 ImageNet-100
    transform = transforms.Compose([
        transforms.Resize(args.image_size),           # Resize shorter side to image_size
        transforms.CenterCrop(args.image_size),       # Crop to square
        transforms.RandomHorizontalFlip(p=0.5),
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])
    ])
    val_tf = transforms.Compose([
        transforms.Resize(args.image_size),           # Resize shorter side
        transforms.CenterCrop(args.image_size),       # Crop to square
        transforms.ToTensor(),
        transforms.Normalize([0.5,0.5,0.5],[0.5,0.5,0.5]),
    ])
    ...
    train_dataset = RobustImageFolder(root=args.data_dir, transform=transform) # to skip corrupted images in imagenet100
    ...
    if os.path.isdir(val_dir):
        ...
        val_dataset = RobustImageFolder(root=val_dir, transform=val_tf) # to skip corrupted images in imagenet100
    ...
    if is_primary(args):
        wandb_logger = wandb.init(
            project='DiffusionRuns', #changed from "ddpm"
            name=args.run_name, 
            config=vars(args))
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, image in enumerate(gen_images):
            ...
            #trying to resize images to fit canvas - DK- 18Nov2025
